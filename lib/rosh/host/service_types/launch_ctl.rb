...
class Rosh
  class Host
    module ServiceTypes
      class LaunchCTL < Rosh::Host::Service
        ...
        def start
          result = @host.shell.exec("launchctl load #{@name}")
        ...
        # @return [Integer,nil]
        def fetch_pid
          pid_result = @host.shell.exec("launchctl list | grep #{@name}")
        ...
        def fetch_status
          ...
          if result.exit_status.zero? && pid
            ...
            [Rosh::UnrecognizedService.new(result.ruby_object),
              result.exit_status, result.ssh_result, pid]
          end
        end
      end
    end
